...
void	handle_client_write(Client &client, const t_server &config)
{
	...
	int written;
	 ...
	 // Check for CGI output first
	if (!client.cgi_output.empty()) {
		std::stringstream sstr;
		sstr << "HTTP/1.1 200 OK\r\n"
			<< "Content-Type: text/html\r\n"
			<< "Content-Length: " << client.cgi_output.size() << "\r\n"
			<< "Connection: keep-alive\r\n"
			<< "\r\n"
			<< client.cgi_output;
		std::string str_response = sstr.str();
		int written = write(client.get_fd(), str_response.c_str(), str_response.size());
		if (written == (int)str_response.size()) {
			printf("CGI WRITE COMPLETE\n____________________________\n");
