...
void	init_config(t_data *d, t_arena *mem)
{
	...
	d->s[0]->error_pages = (const char **)arena_alloc(mem, d->s->error_page_count * sizeof(const char *));
	d->s[0]->error_pages[0] = "/errors/404.html";
	d->s[0]->error_code_count = 1;
	d->s[0]->error_codes = (int *)arena_alloc(mem, d->s->error_code_count * sizeof(int));
	d->s[0]->error_codes[0] = 404;
	d->s[0]->max_bdy_size = 1 * 1024 * 1024; //1mb
	d->s[0]->location_count = 1;
	d->s[0]->locations = (t_location **)arena_alloc(mem, d->s->location_count * sizeof(t_location *));
	...
	d->s[0]->locations[0]->accepted_methods = (const char **)arena_alloc(mem, d->s->locations[0]->method_count * sizeof(const char *));
	...
	d->s[0]->locations[0]->cgi_extensions = (const char **)arena_alloc(mem, d->s->locations[0]->cgi_count * sizeof (const char *));
